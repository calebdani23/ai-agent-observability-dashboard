...
export interface ObservabilityClientOptions { apiUrl: string; appName: string; defaultProvider?: string; defaultModel?: string; defaultSessionId?: string; metadata?: Record<string, unknown>; }
...
export class ObservabilityClient {
  ...
  async sendTrace(traceId: string) { const trace = this.requireTrace(traceId); if (!trace.endedAt) this.finishTrace(traceId); const response = await fetch(`${this.options.apiUrl.replace(/\/$/, "")}/api/traces`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(toBackendPayload(trace)) }); if (!response.ok) throw new Error(`Failed to send trace: ${response.status} ${response.statusText}`); return response.json(); }
