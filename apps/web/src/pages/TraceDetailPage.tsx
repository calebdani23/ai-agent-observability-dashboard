  ...
  return (
    <main>
      ...
      <section className="summary-grid">
        <Card title="Summary"><dl className="details"><dt>App</dt><dd>{t.app_name}</dd><dt>Session</dt><dd>{t.session_id}</dd><dt>Model</dt><dd>{t.provider}/{t.model}</dd><dt>Status</dt><dd><StatusBadge status={t.status} /></dd><dt>Started</dt><dd>{dateTime(t.started_at)}</dd><dt>Latency</dt><dd>{ms(t.latency_ms)}</dd><dt>Tokens</dt><dd>{compact(t.total_tokens)}</dd><dt>Estimated cost</dt><dd>{usd(t.estimated_cost_usd)}</dd>{t.error_message && <><dt>Error</dt><dd>{t.error_message}</dd></>}</dl></Card>
