...
export function DashboardPage() {
  ...
  return (
    <main>
      ...
      {data.total_requests === 0 ? <DatasetEmptyState dataset={dataset} connected={Boolean(auth.user)} hasProviderKey={providerKeys.isLoading ? undefined : Boolean(providerKeys.data?.items.length)} /> : null}
...
function DatasetEmptyState({ dataset, connected, hasProviderKey }: { dataset: TraceDataset; connected: boolean; hasProviderKey?: boolean }) {
  if (dataset === "my_traces" || dataset === "current_openai_session") {
    if (!connected) return <EmptyState message="Signed-out demo visitor"><p className="muted">Sign in to view private live traces, or continue exploring clearly marked demo telemetry.</p><div className="actions compact-actions"><Link className="button primary" to="/sign-in">Sign in</Link><Link className="button secondary" to="/dashboard?dataset=demo">Explore demo instead</Link></div></EmptyState>;
    ...
    if (!hasProviderKey) return <EmptyState message="Save a provider key to start live telemetry."><p className="muted">Your private workspace is empty because no encrypted OpenAI key is saved yet. Save a key, then run your first backend prompt.</p><div className="actions compact-actions"><Link className="button primary" to="/openai-run">Save provider key</Link><Link className="button secondary" to="/dashboard?dataset=demo">Explore demo instead</Link></div></EmptyState>;
    return <EmptyState message="Provider key saved — run your first prompt."><p className="muted">No private traces exist yet. Run an OpenAI prompt to populate your dashboard with live user-owned metrics.</p><div className="actions compact-actions"><Link className="button primary" to="/openai-run">Run first prompt</Link><Link className="button secondary" to="/dashboard?dataset=demo">Explore demo instead</Link></div></EmptyState>;
  }
