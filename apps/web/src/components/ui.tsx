...
export function DataNotice({ source, notice }: { source?: DataSource; notice?: string }) { if (!source) return null; const mode = DEMO_MODE ? "demo fallback enabled" : "live-only mode"; return <div className={`data-notice ${source}`}>{source === "live" ? `Live API data · ${mode}` : notice ?? "Local demo data"}</div>; }
